...
export const SiteNavDropDown: React.FC<SiteNavDropDownProps> = ({ subCategories }) => {
    return (
            ...
            <ul className="p-5 text-black w-full grid grid-cols-2 gap-y-4 gap-x-10">
                {subCategories.map((subCategories) => (
                    <li key={subCategories._id} className="transition text-sm">
                        <Link className="flex items-center gap-3 transition hover:text-rose-600" href={'/'}>{subCategories.title}
