...
export async function syncCartWithProducts(phone: string): Promise<CartSyncWarning[]> {
    ...
    for (let i = cart.items.length - 1; i >= 0; i--) {
        ...
        const newOrig = Number(product.originalPrice);
        const newDisc = Number(product.discountPrice ?? 0);
        ...
        if (stock < 1) {
            ...
            warnings.push({
                ...
                productId: id,
                productName: name,
                message: `${name} is out of stock. Remove it from your cart to continue shopping, or wait until it is restocked.`,
            });
            continue;
        }
        ...
        if (qty > stock) {
            warnings.push({
                ...
                productId: id,
                productName: name,
                message: `${name}: quantity lowered from ${qty} to ${stock} (only ${stock} in stock).`,
            });
            qty = stock;
        }
