...
export const ProductCard = ({
    ...
    return (
        <div
            className={cn(
                "p-3 rounded-2xl bg-white border border-[#0F2744]/15 shadow-[0_8px_24px_rgba(15, 39, 68,0.08)] transition-all duration-300 hover:shadow-[0_14px_30px_rgba(15, 39, 68,0.16)] hover:-translate-y-0.5",
                divCalssName,
        ...
        >
            <div className="w-full relative">
                ...
                >
                        ...
                        images.map((item, index) => (
                            <CarouselItem key={index}>
                                <Link
                                    ...
                                    className={cn(
                                        "block outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-[#0F2744]",
                                        imageContainerClassName
                                    )}
                                >
                                    <Image
                    ...
                    <CarouselPrevious className="left-2 z-20 size-5 text-xs border-[#0F2744]/25 bg-white/90 text-[#0F2744] hover:bg-white" />
                    <CarouselNext className="right-2 z-20 size-5 text-xs border-[#0F2744]/25 bg-white/90 text-[#0F2744] hover:bg-white" />
                </Carousel>
                <div className="absolute bottom-3 left-0 z-10 flex w-full justify-center px-2">
                    {isOutOfStock ? (
                        ...
                        <button
                            type="button"
                            className="text-[#0F2744] md:py-1 py-[2px] px-10 bg-white/90 border border-[#0F2744]/30 rounded-full md:text-xs text-[10px] font-semibold hover:bg-[#0F2744] hover:text-white transition"
                            onClick={() => {
                                setProductId(id);
                                setIsOpen(true);
                                setIsQuickBuy(true);
            ...
            <div className="mt-3 flex flex-col gap-y-1">
               <Link  href={`/product-info/${id}`} className="flex flex-col gap-y-1" >
                <h5 className="text-[0.9rem] font-[700] leading-snug text-[#0F2744] exo line-clamp-2 sm:text-[1rem] md:text-[1.1rem]">{title}</h5>
                <p className="md:text-sm text-xs font-[500] raleway w-full text-[#1B3F66] truncate">
                    {description?.trim() || "Quality products across fashion, home, electronics, and more."}
                </p>
                <div className="flex items-center gap-3">
                    <p className="text-[#0F2744] font-[700] md:text-[1rem] text-sm exo my-1">{"\u20B9"} {price}</p>
                    {
                        discountedPrice && (
