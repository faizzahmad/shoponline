...
export function ShareProductMenu({
    ...
    const channels: ShareChannel[] = useMemo(
        () => [
            ...
            },
            {
                ...
                className:
                    "bg-[#F6F7F9] text-[#0F2744] border border-[#0F2744]/15 hover:bg-[#EEEEEE]",
            },
        ],
    ...
    const triggerStyles =
        triggerVariant === "brand"
            ? "bg-[#0F2744] text-white border border-[#0F2744] shadow-sm hover:bg-[#1B3F66]"
            : "bg-white text-[#0F2744] border border-[#0F2744]/20 shadow-sm hover:bg-[#F6F7F9]";
    ...
    return (
        <div ref={menuRef} className="relative z-[130]">
            <button
                type="button"
                className={cn(
                    "exo flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F2744]/40 sm:gap-2 sm:px-4 sm:text-sm",
                    triggerStyles,
                    triggerClassName
                )}
            ...
            {isOpen ? (
                <div className="absolute right-0 top-full z-[140] mt-2 w-[min(calc(100vw-1.25rem),17rem)] overflow-hidden rounded-xl border border-[#0F2744]/10 bg-white p-0 text-popover-foreground shadow-xl sm:w-[20rem]">
                <div className="flex gap-2 border-b border-[#0F2744]/10 bg-[#F6F7F9] p-2.5 sm:gap-3 sm:p-3.5">
                    {productImage ? (
                        <div className="relative size-14 shrink-0 overflow-hidden rounded-lg border border-[#0F2744]/10 bg-white sm:size-[4.5rem]">
                            <Image
                                src={productImage}
                                alt={productName}
                                fill
                        ...
                        <div className="flex size-14 shrink-0 items-center justify-center rounded-lg border border-dashed border-[#0F2744]/20 bg-white text-[9px] text-[#1B3F66] sm:size-[4.5rem] sm:text-[10px]">
                            No image
                        </div>
                    )}
                    <div className="min-w-0 flex-1">
                        <p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#1B3F66] raleway sm:text-[10px] sm:tracking-[0.14em]">
                            Share product
                        </p>
                        <p className="mt-0.5 line-clamp-2 text-xs font-semibold leading-snug text-[#0F2744] exo sm:text-sm">
                            {productName}
                        </p>
                        {price !== undefined && price !== "" ? (
                            <p className="mt-0.5 text-xs font-bold text-[#0F2744] exo sm:mt-1 sm:text-sm">
                                ₹{price}
                            </p>
                        ) : null}
                    </div>
                ...
                <div className="p-2.5 sm:p-3.5">
                    <p className="text-xs font-semibold text-[#0F2744] exo sm:text-sm">
                        Share with friends
                    </p>
                        ...
                        {channels.map((channel) => {
                            const inner = (
                                <>
                                    ...
                                    <span className="mt-1 block text-center text-[9px] font-medium leading-tight text-[#1B3F66] raleway sm:mt-1.5 sm:text-[10px]">
                                        {channel.label}
                                    </span>
                                </>
                            );
                            ...
                            if (channel.href) {
                                return (
                                    <Link
                                        ...
                                        className="flex flex-col items-center rounded-lg p-0.5 transition hover:bg-[#F6F7F9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F2744]/30 sm:p-1"
